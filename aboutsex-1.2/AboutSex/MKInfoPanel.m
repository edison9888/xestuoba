...
#define TAG_FOR_PANEL           999999          //the panel actually resides in a parent view provide from outside, so invokers should avoid adding other subviews whose tag is equal to TAG_FOR_PANEL.
...
+ (MKInfoPanel *)showPanelInViewNoOverlapping:(UIView*)view type:(MKInfoPanelType)type title:(NSString *)title subtitle:(NSString *)subtitle
{
    UIView* sView = [view viewWithTag:TAG_FOR_PANEL];
    if ([sView isKindOfClass:[MKInfoPanel class]])
    {
        MKInfoPanel* sInfoPanel = (MKInfoPanel*)sView;
        [sInfoPanel performSelector:@selector(removeFromSuperview) withObject:nil afterDelay:0];
    }

    return [self showPanelInView:view type:type title:title subtitle:subtitle];

}

+ (MKInfoPanel *)showPanelInViewNoOverlapping:(UIView*)view type:(MKInfoPanelType)type title:(NSString *)title subtitle:(NSString *)subtitle hideAfter:(NSTimeInterval)interval
{
    UIView* sView = [view viewWithTag:TAG_FOR_PANEL];
    if ([sView isKindOfClass:[MKInfoPanel class]])
    {
    ...
    return [self showPanelInView:view type:type title:title subtitle:subtitle hideAfter:interval];
}
...
//    CGFloat panelHeight = 50;   // panel height when no subtitle set
    CGFloat panelHeight = 29;   // panel height when no subtitle set

    
    panel.type = type;
    panel.titleLabel.text = title;
    panel.titleLabel.textAlignment = UITextAlignmentCenter;
    panel.tag = TAG_FOR_PANEL;
    if(subtitle) {
...
//        panel.titleLabel.frame = CGRectMake(57, 12, 240, 21);
        panel.titleLabel.frame = CGRectMake(57, 5, 240, 21);
    }
