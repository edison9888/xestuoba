...
+ (NSInteger) addOrUpdateStreamItems: (NSMutableArray*)aStreamItemsArray
{
    ...
    NSString* sSQLStrCheckItemExistence = @"SELECT COUNT(*), numVisits FROM stream_items WHERE itemID=?";
    NSString* sSQLStrUpdateNumVisits = @"UPDATE stream_items SET numVisits=? WHERE itemID=?";
    NSString* sSQLStrInsertStreamItem = @"INSERT INTO stream_items(itemID, itemName, location, isRead, isMarked, releasedTime, markedTime, iconURL, summary, numVisits, tag) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
//    BOOL sIsSuccess = [ssFMDatabase executeUpdate:sSQLStrUpdateNumVisits, aDate, [NSNumber numberWithInteger:aItemID]];
    ...
    for(StreamItem* sStreamItem in aStreamItemsArray)
    {
            ...
            && [sItemExistenceCheckRet intForColumnIndex:0] > 0)
        {
            ...
            {    
                [ssFMDatabase executeUpdate:sSQLStrUpdateNumVisits, [NSNumber numberWithInteger:sNumVisits], [NSNumber numberWithInteger:sItemID]];
            }
        ...
        else 
        {
            ...
            [ssFMDatabase executeUpdate:sSQLStrInsertStreamItem, [NSNumber numberWithInteger:sItemID], sItemName, sLocation, [NSNumber numberWithBool:sIsRead], [NSNumber numberWithBool:sIsMarked], sReleasedTime, sMarkedTime, sIconURL, sSummary,[NSNumber numberWithInteger:sNumVisits], [NSNumber numberWithInteger:sTag]]; 
            sItemsInserted++;
        }
...
+ (NSMutableArray*) getLatestStreamItems: (NSInteger) aMaxNumOfItems
{
    [StoreManager openIfNecessary];
    
    NSString* sSQLStr = @"SELECT * FROM stream_items ORDER BY releasedTime DESC LIMIT ?";
    FMResultSet *sItemsResult = [ssFMDatabase executeQuery:sSQLStr, [NSNumber numberWithInteger:aMaxNumOfItems]];
...
+ (BOOL) updateStreamItemReadStatus:(BOOL)aIsRead ItemID:(NSInteger)aItemID
{
    ...
    NSString* sSQLStr = @"UPDATE stream_items SET isRead=? WHERE stream_items.itemID=?";
    BOOL sIsSuccess = [ssFMDatabase executeUpdate:sSQLStr, [NSNumber numberWithBool:aIsRead],[NSNumber numberWithInteger:aItemID]];
...
+ (BOOL) updateStreamItemMarkedStatus:(BOOL)aIsMarked Time:(NSDate*)aDate ItemID:(NSInteger)aItemID
{
    ...
    NSString* sSQLStr = @"UPDATE stream_items SET isMarked=?, markedTime=? WHERE stream_items.itemID=?";
    BOOL sIsSuccess = [ssFMDatabase executeUpdate:sSQLStr, [NSNumber numberWithBool:aIsMarked], aDate, [NSNumber numberWithInteger:aItemID]];
...
+ (BOOL) removeUncollectedStreamItemsIfExceeds:(NSInteger)aMaxCount aMinCount:(NSInteger)aMinCount
{
    ...
    if ([rs next])
    {
        ...
        if (sCount > aMaxCount)
        {
            ...
            BOOL sIsSuccess = [ssFMDatabase executeUpdate:sSQLStr2, [NSNumber numberWithInteger:(sCount-aMinCount)]]; 
            return sIsSuccess;
        }
    }
