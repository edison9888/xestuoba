...
//	_enterPasscodeTableView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    ...
    [_enterPasscodeTableView setBackgroundView:nil];
    [_enterPasscodeTableView setBackgroundColor:[UIColor clearColor]];
...
//	_setPasscodeTableView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    [_setPasscodeTableView setBackgroundView:nil];
    [_setPasscodeTableView setBackgroundColor:[UIColor clearColor]];
...
//	_confirmPasscodeTableView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    [_confirmPasscodeTableView setBackgroundView:nil];
    [_confirmPasscodeTableView setBackgroundColor:[UIColor clearColor]];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (_mode == KKPasscodeModeSet) {
        self.navigationItem.title = NSLocalizedString(@"Set Password", @"");
...
- (void)incrementFailedAttemptsLabel
{
	...
	if (_failedAttemptsCount == 1) {
		_failedAttemptsLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%i Failed Passcode Attempt(s)", @""), _failedAttemptsCount];
	} else {
		_failedAttemptsLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%i Failed Passcode Attempt(s)", @""), _failedAttemptsCount];
	}
...
- (UIView*)headerViewForTextField:(UITextField*)textField
{
    ...
    if (_mode == KKPasscodeModeSet) {
        if ([textField isEqual:_enterPasscodeTextField]) {
            headerLabel.text = NSLocalizedString(@"Enter your passwcode", @"");
        } else if ([textField isEqual:_setPasscodeTextField]) {
