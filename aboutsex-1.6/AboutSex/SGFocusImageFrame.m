...
- (id)initWithFrame:(CGRect)frame delegate:(id<SGFocusImageFrameDelegate>)delegate focusImageArr:(NSArray *)aItems
{
    self = [super initWithFrame:frame];
    if (self) {
        NSMutableArray *imageItems = [NSMutableArray arrayWithArray:aItems];
...
- (void)setupViews
{
    ...
    CGSize size = CGSizeMake(100, 22);
    _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(self.bounds.size.width- size.width, self.bounds.size.height - size.height, size.width, size.height)];
    ...
    */
    ...
    for (int i = 0; i < imageItems.count; i++) {
        ...
        imageView.showsTouchWhenHighlighted = NO;
...
//        imageView.image = item.image;
        ...
        UIView* sBottomView = [[UIView alloc] initWithFrame:CGRectMake(i * _scrollView.frame.size.width, _scrollView.frame.size.height-22.0, _scrollView.frame.size.width, 22.0)];
        sBottomView.backgroundColor = [UIColor grayColor];
        sBottomView.alpha = 0.9;
        
        //添加标题栏
        UILabel * lbltitle = [[UILabel alloc] initWithFrame:CGRectMake(5, 0, sBottomView.bounds.size.width-100, sBottomView.bounds.size.height)];
        ...
        [sBottomView addSubview:lbltitle];
        [lbltitle release];
