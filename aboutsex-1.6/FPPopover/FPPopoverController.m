...
-(id)initWithViewController:(UIViewController*)viewController
{
    self = [super init];
#ifdef DEBUG
    NSLog(@"initWithViewController: the rc of sPopoverController:%d", [self retainCount]);
#endif
...
#ifdef DEBUG
        NSLog(@"initWithViewController2: the rc of sPopoverController:%d", [self retainCount]);
#endif
        [_touchView setTouchedOutsideBlock:^{
            [self dismissPopoverAnimated:YES]; 
        }];
#ifdef DEBUG
        NSLog(@"initWithViewController3: the rc of sPopoverController:%d", [self retainCount]);
#endif
        
        self.contentSize = CGSizeMake(200, 150); //default size
